...
public class UIHealthBars : MaskableGraphic, ILayoutElement {
  ...
  protected override void OnPopulateMesh(VertexHelper vh) {
    ...
    foreach (var hb in StatBar.Active) {
      Vector3 pos = hb.transform.position + new Vector3(0, hb.yOffset, 0);
      DrawHealthBarWorld(vh, camera, canvasRect, index, pos, hb, hb.width, hb.posColor, hb.opacity);

      ++index;
    }
  }

  void DrawHealthBarWorld(VertexHelper vh, Camera camera, Rect r, Int32 index, Vector3 world, StatBar hb, Single width, Color friendly, Single alpha) {
    DrawHealthBarScreen(vh, r, index, camera.WorldToViewportPoint(world), hb, width, friendly, alpha);
  }

  void DrawHealthBarScreen(VertexHelper vh, Rect r, Int32 index, Vector2 screen, StatBar hb, Single width, Color color, Single alpha) {
    Single fill = hb.Percentage();
    ...
    screen.x = r.width * (screen.x) + (float) hb.xOffset;
    screen.y = r.height * (screen.y) + (float) hb.barIdx * Height;
  ...
  void InitArrays() {
    if (_quads_bg == null || _quads_fg == null || _quads_bg.Length < StatBar.Active.Count) {
      _quads_bg = new UIVertex[Math.Max(64, StatBar.Active.Count)][];
