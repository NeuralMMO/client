...
export class EntityView extends Component {
    ...
    start() {
        ...
        this.current_target = this._moveTarget;
        this.tick_delta = 0.0;
    }
    ...
    public UpdateView(data: EntityData): void {
        ...
        if (data.r != data.oR || data.c != data.oC) {
            this._moveTarget = Utils.RCToTileOrigin(data.r, data.c);
            this.last_position = Utils.RCToTileOrigin(data.oR, data.oC);
            this.tick_delta = 0.0;
        }
        else {
    ...
    */
    ...
    public UpdateMove(smooth: boolean = true, delta: number = 0): void {
        ...
        if (smooth) {
            let ratio = math.clamp(this.tick_delta / (GlobalConfig.TickFrac / 1000), 0, 1);
            this.node.position = Vec3.lerp(new Vec3(0, 0, 0), this.last_position, this._moveTarget, ratio);
        }
        else {
            this.node.position = this._moveTarget;
        }
